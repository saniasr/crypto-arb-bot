...
MANUAL_EXCHANGE_MIN_VOLUME_USD = 50_000
...
def check_transferable(status_cache, buy_ex, sell_ex, base_currency):
    ...
    Agar kisi exchange ka status data hi nahi hai (jaise manual exchanges -
    Biconomy, Coinstore, BTSE - jo ye API expose nahi karte), to "unknown"
    maan ke alert bhejte hain (block nahi karte, taaki genuinely available
    opportunities miss na hon), LEKIN alert message me clearly warn karte
    hain ki status verify nahi ho paaya - user ko khud check karna chahiye.
    ...
    unverified = (
        buy_info is None or sell_info is None
        or buy_ex in UNRELIABLE_STATUS_EXCHANGES
        or sell_ex in UNRELIABLE_STATUS_EXCHANGES
    )
...
# TRIANGULAR ARBITRAGE (same exchange ke andar - koi withdraw/deposit
# risk nahi, kyunki coin exchange se bahar hi nahi jaata)
TRIANGULAR_GAP_THRESHOLD_PERCENT = 0.5
# 3 trades hote hain triangular me (2 nahi), isliye fee thoda zyada
TRIANGULAR_FEE_PERCENT = 0.6
TRIANGULAR_MIN_NET_PROFIT_PERCENT = 0.2
# HitBTC ko priority - iske results hamesha pehle dikhenge
PRIORITY_EXCHANGES = {"hitbtc"}


def find_triangular_opportunities(ex_id, full_tickers, threshold_percent):
    """
    Same exchange ke andar triangular loop dhundta hai: USDT -> CoinA -> CoinB
    -> USDT. Agar cross-pair (CoinA/CoinB) ka price, USDT ke through implied
    price se alag hai, to profit banta hai. Koi withdraw/deposit nahi lagta
    kyunki sab kuch ek hi exchange ke andar hota hai - sirf trading fee lagti
    hai (3 trades ki).
    """
    opportunities = []
    for symbol, q in full_tickers.items():
        base, _, quote = symbol.partition("/")
        if quote == QUOTE_CURRENCY or base == QUOTE_CURRENCY:
            continue  # ye already ek USDT pair hai, cross-pair nahi

        base_usdt = full_tickers.get(f"{base}/{QUOTE_CURRENCY}")
        quote_usdt = full_tickers.get(f"{quote}/{QUOTE_CURRENCY}")
        if not base_usdt or not quote_usdt:
            continue  # triangular ke liye teeno pairs chahiye

        try:
            # Path A: USDT -> QUOTE -> BASE -> USDT
            amt_quote = 1.0 / quote_usdt["ask"]
            amt_base = amt_quote / q["ask"]
            final_a = amt_base * base_usdt["bid"]
            profit_a = (final_a - 1.0) * 100
            if 0 < profit_a <= MAX_SANE_GAP_PERCENT and profit_a >= threshold_percent:
                net = profit_a - TRIANGULAR_FEE_PERCENT
                if net >= TRIANGULAR_MIN_NET_PROFIT_PERCENT:
                    opportunities.append({
                        "exchange": ex_id,
                        "path": f"USDT→{quote}→{base}→USDT",
                        "gap_percent": profit_a,
        ...
        try:
            # Path B: USDT -> BASE -> QUOTE -> USDT
            amt_base2 = 1.0 / base_usdt["ask"]
            amt_quote2 = amt_base2 * q["bid"]
            final_b = amt_quote2 * quote_usdt["bid"]
            profit_b = (final_b - 1.0) * 100
            if 0 < profit_b <= MAX_SANE_GAP_PERCENT and profit_b >= threshold_percent:
                net = profit_b - TRIANGULAR_FEE_PERCENT
                if net >= TRIANGULAR_MIN_NET_PROFIT_PERCENT:
                    opportunities.append({
                        "exchange": ex_id,
                        "path": f"USDT→{base}→{quote}→USDT",
                        "gap_percent": profit_b,
                        "net_profit_percent": net,
                    })
        except (KeyError, ZeroDivisionError):
            pass
...
def main():
    ...
    while True:
        ...
        triangular_alerts = []  # is cycle ke triangular opportunities
        for ex_id, ex in exchanges.items():
            usdt_tickers, full_tickers = fetch_all_tickers(ex_id, ex)
            for symbol, price in usdt_tickers.items():
                price_map.setdefault(symbol, {})[ex_id] = price

            # Triangular arbitrage - isi exchange ke andar
            tri_opps = find_triangular_opportunities(
                ex_id, full_tickers, TRIANGULAR_GAP_THRESHOLD_PERCENT
            )
            triangular_alerts.extend(tri_opps)
        ...
        # Triangular arbitrage alerts - HitBTC ko priority (pehle dikhega)
        triangular_alerts.sort(
            key=lambda o: (o["exchange"] not in PRIORITY_EXCHANGES, -o["net_profit_percent"])
        )
        tri_sent = 0
        for opp in triangular_alerts[:10]:  # zyada se zyada 10 triangular alert
            key = ("TRIANGULAR", opp["exchange"], opp["path"])
            now = time.time()
            if key in last_alert_time and (now - last_alert_time[key]) < ALERT_COOLDOWN_SECONDS:
                continue
            last_alert_time[key] = now
            star = "⭐ " if opp["exchange"] in PRIORITY_EXCHANGES else ""
            msg = (
                f"🔺 {star}<b>Triangular Arbitrage: {opp['exchange']}</b>\n\n"
                f"Path: <b>{opp['path']}</b>\n"
                f"Raw Gap: <b>{opp['gap_percent']:.2f}%</b>\n"
                f"Est. Net Profit (after ~{TRIANGULAR_FEE_PERCENT}% fees): "
                f"<b>{opp['net_profit_percent']:.2f}%</b>\n"
                f"✅ Same exchange - no withdraw/deposit needed"
            )
            print(msg.replace("\n", " | "))
            send_telegram_alert(msg)
            tri_sent += 1
        ...
        print(f"[Cycle] done in {elapsed:.1f}s, {len(detailed)} detailed + "
              ...
              f"(withdraw/deposit disabled), {tri_sent} triangular sent "
              f"({len(triangular_alerts)} found)")
