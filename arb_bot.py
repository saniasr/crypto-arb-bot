...
def load_currency_status(exchanges):
    ...
    rarely change hota hai). Agar exchange ye support nahi karta to us
    exchange ke liye status "unknown" treat hoga (check skip ho jayega).

    Returns: {exchange_id: {currency_code: {"deposit": bool, "withdraw": bool}}}
    """
    status_cache = {}
...
def check_transferable(status_cache, buy_ex, sell_ex, base_currency):
    ...
    Agar kisi exchange ka status data hi nahi hai, to "unknown" maan ke
    aage badhne dete hain (check skip) - warna manual exchanges (Biconomy
    etc) ke saare alerts hamesha block ho jayenge.
    """
    ...
    if buy_info and buy_info["withdraw"] is False:
        return False, f"withdraw disabled on {buy_ex}"
    if sell_info and sell_info["deposit"] is False:
        return False, f"deposit disabled on {sell_ex}"
    return True, None
...
def remove_frozen_prices(price_history, price_map, freeze_cycles=3):
    """
...
def main():
    ...
    while True:
        ...
        for gap in all_alerts:
            ...
            can_transfer, reason = check_transferable(
        ...
        print(f"[Cycle] done in {elapsed:.1f}s, {sent_count} alert(s) sent, "
              f"{skipped_no_transfer} skipped (withdraw/deposit disabled)")
